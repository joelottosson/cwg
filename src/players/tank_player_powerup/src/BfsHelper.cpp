...
std::pair<int, int> BfsHelper::MoveDown(const std::pair<int, int>& pos) const
{
    ...
    bool BfsHelper::CanWrapX()
    {
        for (int y = 0; y < m_SizeY; y++) {
            if (CanReachSquare(std::make_pair(0, y)) && CanReachSquare(std::make_pair(m_SizeX - 1, y))) {
                return true;
            }
        }
        return false;
    ...
    bool BfsHelper::CanWrapY()
    {
        for (int x = 0; x < m_SizeX; x++) {
            if (CanReachSquare(std::make_pair(x, 0)) && CanReachSquare(std::make_pair(x, m_SizeY - 1))) {
                return true;
            }
        }
        return false;
